...
$(document).ready(function(){
	...
	function setStatus(code, message){
		...
		if ((code != null && code != undefined && code != '')
			...
			if (code == 1){
				...
				status.html('Можно писать без дефисов, пробелов и точек: Уб1810б');
			}
		}
	}
	...
	function makeTimetable(callback){
		...
		$.getJSON(BASE_API_URL + groupName, function(data, status){
			...
			connectionError(1, 'Проблемы с интернетом. Попробуйте переотправить запрос или перезагрузить страницу');
